...
public final class JsonParser2 extends Transform<StructuredRecord, StructuredRecord> implements Serializable {
  private static final Logger LOG = LoggerFactory.getLogger(JsonParser2.class);
  ...
  public JsonParser2(Config config) {
    this.config = config;
  }
  ...
  public void transform(StructuredRecord input, Emitter<StructuredRecord> emitter) throws Exception {
    ...
    if (isSimple) {
      // TODO: Handle input as string instead and it should go through another transform?
      emitter.emit(StructuredRecordStringConverter.fromJsonString(new String((byte[])input.get(config.field)), outSchema));
      return;
    }
