...
class KubeTwillController implements ExtendedTwillController {
  ...
  private void deleteJob() {
    ...
    try {
      ...
      LOG.warn("Error while deleting job {} or corresponding pod(s).", name, e);
    }
  }

  private void deletePod(String jobName) {
    try {
      ...
      LOG.warn("Failed to delete pods for job {}", jobName, e);
    }
  }
  ...
  private CompletableFuture<KubeTwillController> cleanupJob() {
    CompletableFuture<KubeTwillController> resultFuture;
    CompletionStage<String> completionStage;
    resultFuture = completion.thenApply(r -> r);
    // get job status
    completionStage = getJobCompletionStatus();
