...
public class InternalAccessEnforcer extends AbstractAccessEnforcer {
  ...
  private void validateAccessTokenAndIdentity(String principalName, Credential credential) throws AccessException {
    ...
    if (!userIdentity.getUsername().equals(principalName)) {
      throw new AccessException(String.format("Internal access token username differs from principal name; got token " +
    ...
    if (!userIdentity.getUsername().equals(SystemAuthenticationContext.SYSTEM_IDENTITY)) {
      throw new AccessException(String.format("Unexpected principal name; got '%s', want '%s'",
                                              userIdentity.getUsername(), SystemAuthenticationContext.SYSTEM_IDENTITY));
    }
  }
}
