...
public class InternalAccessEnforcerTest {
  ...
  @Test(expected = AccessException.class)
  public void testInternalAccessEnforceUserDiscrepancyFailure() throws IOException {
    NamespaceId ns = new NamespaceId("namespace");
    long currentTime = System.currentTimeMillis();
    UserIdentity userIdentity = new UserIdentity(SYSTEM_PRINCIPAL, UserIdentity.IdentifierType.INTERNAL,
                                                 Collections.emptyList(), currentTime,
    ...
    String encodedIdentity = Base64.getEncoder()
      .encodeToString(accessTokenCodec.encode(tokenManager.signIdentifier(userIdentity)));
    Credential credential = new Credential(encodedIdentity, Credential.CredentialType.INTERNAL);
    Principal principal = new Principal("other-principal", Principal.PrincipalType.USER, null, credential);
    internalAccessEnforcer.enforce(ns, principal, StandardPermission.GET);
  }

  @Test(expected = AccessException.class)
  public void testInternalAccessEnforceOnParentUserDiscrepancyFailure() throws IOException {
    NamespaceId ns = new NamespaceId("namespace");
    long currentTime = System.currentTimeMillis();
    UserIdentity userIdentity = new UserIdentity(SYSTEM_PRINCIPAL, UserIdentity.IdentifierType.INTERNAL,
                                                 Collections.emptyList(), currentTime,
  ...
  @Test
  public void testInternalAccessIsVisibleUserDiscrepancyFailure() throws IOException {
    NamespaceId ns = new NamespaceId("namespace");
    Set<EntityId> entities = Collections.singleton(ns);
    long currentTime = System.currentTimeMillis();
    UserIdentity userIdentity = new UserIdentity(SYSTEM_PRINCIPAL, UserIdentity.IdentifierType.INTERNAL,
  ...
  @Test(expected = AccessException.class)
  public void testInternalAccessEnforceNonSystemPrincipal() throws IOException {
    NamespaceId ns = new NamespaceId("namespace");
    long currentTime = System.currentTimeMillis();
    String nonSystemPrincipal = "not-system";
    UserIdentity userIdentity = new UserIdentity(nonSystemPrincipal, UserIdentity.IdentifierType.INTERNAL,
                                                 Collections.emptyList(), currentTime,
                                                 currentTime + 5 * MINUTE_MILLIS);
    String encodedIdentity = Base64.getEncoder()
      .encodeToString(accessTokenCodec.encode(tokenManager.signIdentifier(userIdentity)));
    Credential credential = new Credential(encodedIdentity, Credential.CredentialType.INTERNAL);
    Principal principal = new Principal(nonSystemPrincipal, Principal.PrincipalType.USER, null, credential);
    internalAccessEnforcer.enforce(ns, principal, StandardPermission.GET);
  }

  @Test(expected = AccessException.class)
  public void testInternalAccessEnforceOnParentNonSystemPrincipal() throws IOException {
    NamespaceId ns = new NamespaceId("namespace");
    long currentTime = System.currentTimeMillis();
    String nonSystemPrincipal = "not-system";
    UserIdentity userIdentity = new UserIdentity(nonSystemPrincipal, UserIdentity.IdentifierType.INTERNAL,
  ...
  @Test
  public void testInternalAccessIsVisibleNonSystemPrincipal() throws IOException {
    NamespaceId ns = new NamespaceId("namespace");
    Set<EntityId> entities = Collections.singleton(ns);
    long currentTime = System.currentTimeMillis();
    String nonSystemPrincipal = "not-system";
